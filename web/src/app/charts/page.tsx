...
export default function ChartsPage() {
  ...
  const loadChart = async (sym: string, tf: string, signal?: AbortSignal) => {
    const { period, interval } = TIMEFRAMES[tf];
    const cacheKey = `niveshai_chart_cache_${sym}_${tf}`;
    ...
    if (cached) {
      try {
        ...
        if (Date.now() - timestamp < 300000) { // 5 min TTL
           ...
           // Silent background refresh? Only if not already loading
           // For simplicity in this demo, we skip silent refresh if cache is fresh
           return;
        }
      } catch (e) {}
    }
    ...
    try {
      const res = await fetch(`/api/market?symbol=${sym}&period=${period}&interval=${interval}`, { signal });
  ...
  const loadPatterns = async (sym: string, signal?: AbortSignal) => {
    const cacheKey = `niveshai_patterns_cache_${sym}`;
    const cached = localStorage.getItem(cacheKey);
    ...
    setMessages([]); setSelectedPattern(null);

    try {
      const res = await fetch(`/api/patterns?symbol=${sym}`, { signal });
      const data = await res.json();
      if (!data.error) {
        setPatternData(data);
  ...
  const sendMessage = async (userMsg: string, pattern?: any) => {
    if (!apiKey || apiKey.includes('your_gemini_api_key_here')) {
      // Provide high-quality fallback for Demo Mode
      const p = pattern || patternData?.patterns?.[0];
      const name = symbol.replace('.NS', '');
      let fallbackText = `**Demo Mode Analysis for ${name}**\n\n`;
      
      if (p) {
        fallbackText += `I've detected a **${p.name}** pattern which has a **${p.confidence}% confidence score**. Historically, this setup on ${name} suggests a potential ${p.type === 'BULLISH' ? 'upside move' : 'downside risk'}. \n\n**Action for retail:** Keep a close eye on the support level at ₹${patternData?.support}. If the price stays above this, the bullish momentum could continue.`;
      } else {
        fallbackText += `The current chart for ${name} shows solid structural support at **₹${patternData?.support}** and resistance at **₹${patternData?.resistance}**. The price action is currently stabilizing as the market looks for a catalyst.`;
      }
